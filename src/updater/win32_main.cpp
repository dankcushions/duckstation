...
static std::string WideCharStringToStdString(const PWSTR str)
{
  const size_t wide_len = std::wcslen(str);
  int u8len = WideCharToMultiByte(CP_UTF8, 0, str, static_cast<int>(wide_len), nullptr, 0, nullptr, nullptr);
  std::string result;
  if (u8len > 0)
  {
    result.resize(u8len);
    u8len = WideCharToMultiByte(CP_UTF8, 0, str, static_cast<int>(wide_len), result.data(),
                                static_cast<int>(result.size()), 0, nullptr);
    if (u8len > 0)
      result.resize(u8len);
    else
      result.clear();
...
int wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nShowCmd)
{
  ...
  const int parent_process_id = StringUtil::FromChars<int>(WideCharStringToStdString(argv[0])).value_or(0);
  const std::string destination_directory = WideCharStringToStdString(argv[1]);
  const std::string zip_path = WideCharStringToStdString(argv[2]);
  const std::string program_to_launch = WideCharStringToStdString(argv[3]);
  LocalFree(argv);
