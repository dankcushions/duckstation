...
unsigned char PGXP_GetVertex(u32 addr, u32 value, int x, int y, int xOffs, int yOffs, float* out_x, float* out_y, float* out_w)
{
    ...
    const short psx_y = (short)(value >> 16);
		...
		if ((vert) && /*(IsSessionID(vert->count)) &&*/ (vert->gFlags == 1))
		{
			...
			py = vert->y + yOffs;
			pw = vert->z / 32768.0f;
			valid_w = 0;	// iCB: Getting the wrong w component causes too great an error when using perspective correction so disable it
		}
		else
		{
